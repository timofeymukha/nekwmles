...
!! @remark This routine uses global scratch space \a SCRMG, \a SCRUZ, \a SCRNS, \a SCRSF
      ...
      common /SCRNS/ dudx, dvdx, tmptemp
      common /SCRSF/ dwdx

      real dtdx(LX1,LY1,LZ1,LELT,3) ! dt/dx, dt/dy and dt/dz
      real sltemp(LX1, LY1, LZ1, LELT)   !work array for temperature
      common /CTMP1/ sltemp
...
!-----------------------------------------------------------------------
      ...
      call user_stat_trnsv(tmpvel,dudx,dvdx,dwdx,dtdx,slvel,tmppr,
     $                     tmptemp)
     ...
     $     tmpvel(1,1,1,1,3),NELV)
      ...
      ! temperature
      call stat_reshufflev(sltemp,tmptemp,NELT)
     ...
     $     dvdx(1,1,1,1,3),NELV)
      ...
      ! reshuffle temperature derivatives
      ! 
      call stat_reshufflev(tmpvel(1,1,1,1,1),dtdx(1,1,1,1,1),NELT)
      call stat_reshufflev(tmpvel(1,1,1,1,2),dtdx(1,1,1,1,2),NELT)
      if (if3d) then
         call stat_reshufflev(tmpvel(1,1,1,1,3),dtdx(1,1,1,1,3),NELT)
      endif


      ! copy
      itmp = LX1*LY1*LZ1*LELT*LDIM
      call copy(dtdx,tmpvel,itmp)
...
!-----------------------------------------------------------------------
      ...
      call stat_reshufflev(tmppr(1,1,1,1),vdiff(1,1,1,1,1),NELV)
...
!-----------------------------------------------------------------------
      ...
      lnvar = lnvar + 1
      npos = lnvar
      call stat_compute_1Dav1(sltemp(1,1,1,1),npos,alpha,beta)

      ! <(tt)>t
      lnvar = lnvar + 1
      npos = lnvar
      call stat_compute_1Dav2(sltemp(1,1,1,1),sltemp(1,1,1,1),
     $     npos,alpha,beta)

      ! <(tu)>t
      lnvar = lnvar + 1
      npos = lnvar
      call stat_compute_1Dav2(sltemp(1,1,1,1),slvel(1,1,1,1,1),
     $     npos,alpha,beta)

      ! <(tv)>t
      lnvar = lnvar + 1
      npos = lnvar
      call stat_compute_1Dav2(sltemp(1,1,1,1),slvel(1,1,1,1,2),
     $     npos,alpha,beta)

      ! <(tw)>t
      lnvar = lnvar + 1
      npos = lnvar
      call stat_compute_1Dav2(sltemp(1,1,1,1),slvel(1,1,1,1,3),
     $     npos,alpha,beta)
      ...
      lnvar = lnvar + 1
      npos = lnvar
      call stat_compute_1Dav1(dtdx(1,1,1,1,1),
     $     npos,alpha,beta)

      ! <dtdy)>t
      lnvar = lnvar + 1
      npos = lnvar
      call stat_compute_1Dav1(dtdx(1,1,1,1,2),
     $     npos,alpha,beta)

      ! <dtdz)>t
      lnvar = lnvar + 1
      npos = lnvar
      call stat_compute_1Dav1(dtdx(1,1,1,1,3),
     $     npos,alpha,beta)

      ! reshuffle eddy diffusivity
      call stat_reshufflev(tmptemp(1,1,1,1),vdiff(1,1,1,1,2),NELT)

      ! <k_sgs*dtdx)>t, here k_sgs is sgs diffusivity
      lnvar = lnvar + 1
      npos = lnvar
c      call stat_compute_1Dav2(dtdx(1,1,1,1,1),
c     $     npos,alpha,beta)
      call stat_compute_1Dav2(tmptemp(1,1,1,1),dtdx(1,1,1,1,1),
     $     npos,alpha,beta)

      ! <k_sgs*dtdy)>t
      lnvar = lnvar + 1
      npos = lnvar
      call stat_compute_1Dav2(tmptemp(1,1,1,1),dtdx(1,1,1,1,2),
     $     npos,alpha,beta)

      ! <k_sgs*dtdz)>t
      lnvar = lnvar + 1
      npos = lnvar
      call stat_compute_1Dav2(tmptemp(1,1,1,1),dtdx(1,1,1,1,3),
     $     npos,alpha,beta)

      ! <k_sgs)>t
      lnvar = lnvar + 1
      npos = lnvar
      call stat_compute_1Dav1(tmptemp(1,1,1,1),npos,alpha,beta)

!-----------------------------------------------------------------------
      ! <tau>t (we use the tmpvel array for the tau field components)

      call stat_reshufflev(tmpvel(1,1,1,1,1),wmles_tau_field(1,1,1,1,1),
     $                     NELV)
      call stat_reshufflev(tmpvel(1,1,1,1,2),wmles_tau_field(1,1,1,1,2),
     $                     NELV)
      if (if3d) call stat_reshufflev(tmpvel(1,1,1,1,3),
     $     wmles_tau_field(1,1,1,1,3),NELV)
...
c      call stat_compute_1Dav2(slvel(1,1,1,1,1),slvel(1,1,1,1,1),
c     $     npos,alpha,beta)
!-----------------------------------------------------------------------
